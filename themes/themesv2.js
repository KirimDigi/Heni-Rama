...
// loader
function hideInvitationLoader() {
  var loader = document.getElementById("loader");
  var sm = document.getElementById("satuMomen");
  if (loader) {
...
if (document.readyState === "complete" || document.readyState === "interactive") {
  setTimeout(hideInvitationLoader, 50);
} else {
  document.addEventListener("DOMContentLoaded", hideInvitationLoader);
  window.addEventListener("load", hideInvitationLoader);
}
// Fallback timeout
setTimeout(hideInvitationLoader, 1500);
...
var openInvitation = function openInvitation(event) {
  ...
  // play music starting from 02:51 (171 seconds)
  var musicEl = document.getElementById("music");
  if (musicEl) {
    musicEl.muted = false;
    musicEl.volume = 1;
    var targetStart = 171; // 02:51
    var applyTime = function () {
      try {
        musicEl.currentTime = targetStart;
      } catch (e) {}
    };
    if (musicEl.readyState >= 1) {
      applyTime();
    } else {
      musicEl.addEventListener("loadedmetadata", applyTime, { once: true });
    }
    musicEl.addEventListener("playing", function () {
      if (musicEl.currentTime < 165) {
        applyTime();
      }
    }, { once: true });
  }
